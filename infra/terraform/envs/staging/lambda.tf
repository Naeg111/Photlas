# staging Lambda 3関数（決定 G：infra のみ TF 管理・コードは ignore_changes）
# コードは scripts/deploy-lambda.sh でデプロイ。filename はプレースホルダ。env の機密は変数。

resource "aws_lambda_function" "moderation_scanner" {
  function_name = "photlas-moderation-scanner-test"
...
resource "aws_lambda_function" "moderation_monitor" {
  function_name = "photlas-moderation-monitor-test"
  role          = aws_iam_role.lambda_moderation.arn
  runtime       = "python3.12"
  handler       = "lambda_function.lambda_handler"
  architectures = ["x86_64"]
...
resource "aws_lambda_function" "thumbnail_generator" {
  function_name = "photlas-thumbnail-generator-test"
