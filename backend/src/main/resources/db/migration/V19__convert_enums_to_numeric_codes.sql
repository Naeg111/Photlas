...
-- Step 2: 旧ID→新IDマッピングを作成し、photo_categoriesを先に更新
CREATE TEMPORARY TABLE cat_map (old_id INTEGER, new_id INTEGER);
INSERT INTO cat_map SELECT category_id, CASE name
...
UPDATE photo_categories pc SET category_id = m.new_id
FROM cat_map m WHERE pc.category_id = m.old_id;

-- Step 3: categoriesのPKを直接UPDATE（旧ID 1-14 → 新ID 201-214、範囲が重複しないため安全）
UPDATE categories c SET category_id = m.new_id FROM cat_map m WHERE c.category_id = m.old_id;
DROP TABLE cat_map;
