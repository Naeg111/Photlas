...
describe('OAuthSignUpDialog', () => {
  ...
  it('「戻る」ボタンで onBack が呼ばれる（SignUpMethodDialog に戻る）', async () => {
    const onBack = vi.fn()
    renderDialog({ onBack })
  ...
  it('「戻る」ボタンは w-full クラスを持つ（SignUpDialog の戻る+登録 合計幅と同等）', () => {
    renderDialog()
    const cancelBtn = screen.getByRole('button', { name: '戻る' })
